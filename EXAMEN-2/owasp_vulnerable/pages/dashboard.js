...
export default function Dashboard() {
  ...
  return (
    <div>
      ...
      {/* CWE-89: SQL Injection - Búsqueda de usuarios */}
      <div style={{marginTop: '30px'}}>
        <h2><span style={{color: '#ff3366'}}>●</span> CWE-89: SQL Injection - Búsqueda</h2>
        <div className="card" style={{background: 'rgba(255, 50, 50, 0.1)', border: '1px solid rgba(255, 100, 100, 0.3)'}}>
          <p style={{color: '#ff9999', fontSize: '13px', marginBottom: '15px'}}>
            <strong>⚠️ VULNERABILIDAD:</strong> Esta búsqueda concatena directamente el input en la consulta SQL.
            Prueba: <code>' OR '1'='1</code> o <code>'; DROP TABLE users; --</code>
          </p>
      ...
      {/* CWE-79: XSS - Comentarios */}
      <div style={{marginTop: '30px'}}>
        <h2><span style={{color: '#ff3366'}}>●</span> CWE-79: Cross-Site Scripting (XSS)</h2>
        <div className="card" style={{background: 'rgba(255, 50, 50, 0.1)', border: '1px solid rgba(255, 100, 100, 0.3)'}}>
          <p style={{color: '#ff9999', fontSize: '13px', marginBottom: '15px'}}>
            <strong>⚠️ VULNERABILIDAD:</strong> Los comentarios se renderizan sin sanitizar.
            Prueba: <code>&lt;img src=x onerror=alert('XSS')&gt;</code> o <code>&lt;script&gt;alert('XSS')&lt;/script&gt;</code>
          </p>
          <textarea
            value={newComment}
          ...
          <div style={{marginTop: '20px'}}>
            ...
            {comments.map(c => (
              <div key={c.id} style={{marginTop: '10px', padding: '10px', background: 'rgba(0,0,0,0.3)', borderRadius: '4px'}}>
                ...
                {/* VULNERABILIDAD: dangerouslySetInnerHTML sin sanitizar */}
      ...
      {/* CWE-73: Path Traversal - Descarga de archivos */}
      <div style={{marginTop: '30px'}}>
        <h2><span style={{color: '#ff3366'}}>●</span> CWE-73: Path Traversal</h2>
        <div className="card" style={{background: 'rgba(255, 50, 50, 0.1)', border: '1px solid rgba(255, 100, 100, 0.3)'}}>
          <p style={{color: '#ff9999', fontSize: '13px', marginBottom: '15px'}}>
            <strong>⚠️ VULNERABILIDAD:</strong> El endpoint de descarga no valida el path.
            Prueba: <code>../secret.txt</code> o <code>../../package.json</code>
          </p>
          <div style={{display: 'flex', gap: '10px', alignItems: 'center'}}>
          ...
          <div style={{marginTop: '10px', fontSize: '12px', color: '#ffaa00'}}>
            Archivos válidos: document1.txt, document2.txt<br/>
            Archivos protegidos: ../secret.txt (¡intenta acceder!)
          </div>
        </div>
      </div>
      ...
      {role === 'admin' && (
        <div>
          ...
          <div style={{marginBottom: '20px', padding: '15px', background: 'rgba(255, 170, 0, 0.1)', border: '1px solid rgba(255, 170, 0, 0.3)', borderRadius: '6px'}}>
            <p style={{margin: '0', fontSize: '13px', color: '#ffcc66', lineHeight: '1.6'}}>
              <strong>⚠️ ADVERTENCIA:</strong> Esta sección contiene vulnerabilidades IDOR (Insecure Direct Object References). 
              Los controles de acceso pueden ser manipulados para acceder a recursos no autorizados.
            </p>
          </div>
          ...
          <div style={{marginTop: '25px', padding: '15px', background: 'rgba(255, 0, 50, 0.1)', border: '1px solid rgba(255, 50, 100, 0.3)', borderRadius: '6px'}}>
            <p style={{margin: '0', fontSize: '12px', color: '#ff6688', lineHeight: '1.6'}}>
              <strong>🔓 VULNERABILIDAD DETECTADA:</strong> Esta funcionalidad no verifica correctamente los permisos del usuario. 
              Un atacante podría manipular las peticiones API directamente para modificar roles sin autorización (Broken Access Control).
            </p>
          </div>
        </div>
      )}
